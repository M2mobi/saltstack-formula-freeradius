...
We will see a quick setup to configure the ``freeradius`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``freeradius`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``freeradius`` formulas under ``/srv/salt/freeradius/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/freeradius/parameters/dns:domain/
    ln -s dns:domain /srv/salt/freeradius/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/freeradius/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/freeradius/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``freeradius`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``freeradius`` formulas under ``/srv/salt/freeradius/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/freeradius/parameters/roles
...
We create a configuration for the role ``freeradius/server`` in ``/srv/salt/freeradius/parameters/roles/freeradius/server.yaml``:
...
We create another configuration for the role ``freeradius/client`` in ``/srv/salt/freeradius/parameters/roles/freeradius/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles freeradius/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles freeradius/client
...
.. code-block:: console
    ...
    mv /srv/salt/freeradius/parameters/dns:domain/  '/srv/salt/freeradius/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-freeradius-as-json:
      test.nop:
        - name: {{ freeradius | json }}
...
Here is an example based on `template-formula/freeradius/config/file.sls`_:
...
This ``sls`` file expose a ``freeradius`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ freeradius['winner'] }}
...
.. _template-formula/freeradius/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/freeradius/config/file.sls
